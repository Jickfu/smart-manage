...
$backendPomPath = Join-Path $repositoryRoot 'pom.xml'
$platformPomPath = Join-Path $repositoryRoot 'platform/pom.xml'
$migrationDirectory = Join-Path $repositoryRoot 'platform/src/main/resources/db/platform/migration'
...
try {
    ...
    Invoke-Psql $verifyDatabase @('-v', 'ON_ERROR_STOP=1', '-c', "SELECT 1 / (CASE WHEN count(*) = 1 THEN 1 ELSE 0 END) AS single_company FROM t_sys_org;")
