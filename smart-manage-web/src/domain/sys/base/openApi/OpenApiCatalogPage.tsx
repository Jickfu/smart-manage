...
import { downloadBlob } from '@/domain/common/dataExchange/fileDownload';
...
const OpenApiCatalogPage = (props: PageComponentProps) => {
  ...
  const exportMutation = useCommandMutation({
    mutationFn: () => openApiPlatformApi.catalogExport(selectedKeys.map(String)),
    onSuccess: (blob) => {
      downloadBlob(blob, 'API接口文档.md');
      feedback.success('API 文档导出成功');
    },
  });
      ...
      render: (value: string, release) => (
        <Button
          ...
          onClick={() =>
            openBillTab(
              props.appNumber,
              componentKeys.openApiCatalogDetail,
              release.id,
              OperationType.VIEW,
            )
          }
      ...
      toolbarActions={[
        {
          key: 'export',
          label: '导出 Markdown',
          permission: openApiCatalogAccess.permissions.listPage,
