...
- 采用“标题栏、内容区、底部操作区”三段式结构的业务弹框必须复用 `src/domain/common/component/AppModal.tsx`：标题位于左侧，搜索或筛选等扩展操作通过 `headerExtra` 提供，关闭使用壳层统一的 Ant Design 原生关闭按钮，内容滚动通过 `bodyMode` 控制，底部操作通过 `footer` 提供。业务组件不得重复覆盖 Modal 的容器、标题栏、关闭按钮和底部操作区样式；简单提示、确认和结果反馈弹框可以直接使用 Ant Design Modal。
