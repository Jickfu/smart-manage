...
- 先通过现有代码、文档和用户已明确的要求消除疑问；仍存在影响架构方向、数据安全、外部状态或产品行为的实质性决策时，必须询问用户。已明确授权的范围无需重复确认。
- 已授权的实施任务应持续完成相关实现与验证；本次改动造成的失败应修复并复验，遇到需要用户决策或无法解决的环境阻塞时说明。
...
按下表读取必要总则及本次涉及的章节，跨层或系统性变更再扩大阅读范围。已加载且未变化的规则无需重复读取。小范围修复、纯重命名或不改变模块边界的局部调整无需完整模块流程。
