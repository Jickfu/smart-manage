...
class DomainFlywayMigrationStrategyTests {
    @Test
    void disabledBusinessKeepsExistingPlatformOnlyBehavior() {
        Flyway platform = mock(Flyway.class);
        new DomainFlywayMigrationStrategy(null).migrate(platform);
    ...
    void sharedHistoryOrOverlappingLocationsAreRejectedBeforeAnyMigration() {
        ...
        when(configuration.getTable()).thenReturn("flyway_test_domain_schema_history");
    ...
    void invalidRequiredVersionIsRejectedBeforePlatformMutation() {
        ...
        for (String version : new String[]{"", "0", "latest", "-1"}) {
            DomainMigration properties = new DomainMigration("test", "classpath:db/test-domain", "flyway_test_domain_schema_history", version);
            assertThrows(IllegalArgumentException.class,
                    () -> new DomainFlywayMigrationStrategy(properties).migrate(platform));
        }
        verify(platform, never()).migrate();
    }

    private static DomainFlywayMigrationStrategy strategy() {
        return new DomainFlywayMigrationStrategy(new DomainMigration("test", "classpath:db/test-domain", "flyway_test_domain_schema_history", "2"));
    }
    ...
    private static Flyway platform() {
        ...
        when(configuration.getTable()).thenReturn("flyway_schema_history");
        when(configuration.getLocations()).thenReturn(new Location[]{new Location("classpath:db/platform/migration")});
        return platform;
    }
}
