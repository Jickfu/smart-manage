...
- 共享会话与一次性票据使用 Redis；用户启用状态、凭据代际及角色权限以 PostgreSQL 为鉴权权威，每次受保护请求直接校验，不通过缓存删除维持撤权正确性。进程内缓存只允许保存可丢弃、可重建且允许节点间短暂不一致的数据，并必须定义版本、短 TTL 或跨节点失效机制。
- 用户认证状态使用 Redis。
...
- 一次性凭据制品在打开文件流前，必须校验当前用户为所有者、制品有效且可下载，并通过带并发版本检查的事务更新原子占用下载资格；占用失败不得开始传输。完成或释放操作必须匹配同一次下载声明，不能处理其他请求的占用。
- 打开文件或流传输明确失败时，当前实现允许释放同次下载资格；内容已完整交给响应流后，完成状态写入失败不得释放资格。服务端传输完成不等于确认客户端收到文件，不得以客户端未确认接收为由重新开放一次性秘密。
- 超时仍未完成的一次性凭据下载声明必须按结果不确定处理，使制品失效并进入待清理状态，不得套用普通制品的超时恢复逻辑重新开放下载。
