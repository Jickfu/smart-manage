...
保持一个前端工程、一次构建和统一部署。未设置 `SMART_MANAGE_DOMAINS`（或设为 `all`）时，构建脚本自动发现 `src/domain` 下当前存在的领域目录，忽略公共目录并始终要求 `sys`；仍支持 `sys`、`sys,demo` 等显式发行组合。新增或删除领域目录无需维护第二份前端清单。仓库门禁会核对前后端当前领域集合，防止只改一端。
...
提交的生成文件始终对应当前仓库的默认全部领域。DEMO 专属组件键和应用首页在 DEMO 目录内维护；公共注册器只通过生成文件导入本次所选业务。

### 构建产物与分块

Vite 使用 `build.rolldownOptions` 配置输出，文件保留内容哈希。目录表示资源归属，不作为强制合包边界：

| 输出目录 | 归属 |
| --- | --- |
| `assets/shell/` | 管理端入口脚本 |
| `assets/domains/{领域}/{应用}/` | 能确定唯一应用归属的页面、首页和应用资源 |
| `assets/shared/` | 跨应用代码、公共组件及无法确定唯一应用归属的资源 |
| `assets/vendor/` | 第三方依赖代码 |

页面和首页保留独立动态入口；不按源码文件逐一强制拆包，也不把整个领域或应用合为一个 JS。CSS 等资源按构建器提供的原始文件信息归类，跨应用合并或无法确定唯一应用归属时进入 `shared`，同一应用的多个 CSS 来源仍可归入该应用目录；`public` 中的静态资源保持原有路径。

React 和数据请求库保持独立公共分组。其余第三方依赖仅对 `$initial`（本来已被入口静态引用的模块）按组件运行库与其余基础依赖归并，以减少初始请求数；图表、代码编辑器及未使用的组件继续按需加载。分块优先比较典型访问路径的请求数、去重后的体积和缓存复用，不能仅以 `dist` 文件总数或单个入口文件大小判断性能。用户配置图标使用独立的版本化矢量数据资源，不进入逐图标 JavaScript 模块依赖图；静态界面图标仍按具体组件导入。

每次构建自动核验最终 JS 依赖图，禁止未选领域混入、注册页面/首页失去异步入口或目录归属、图表/代码编辑器进入初始依赖链。`dist/.vite/manifest.json` 提供资源映射，`dist/.vite/build-report.json` 记录领域、异步入口数量及入口静态依赖闭包的 JS 文件数、原始字节和逐文件 gzip 字节总和。报告不包含 CSS、`public` 脚本、业务接口或运行时触发的动态请求，不等同于真实网络耗时。
...
- 页签容量按单个浏览器标签页内、跨应用的实际保活页面总数管理：系统固定入口、消息中心和未加载的固定应用标签不计数；每个已加载应用首页及业务、临时编辑、工作台 iframe 页面各计一个。达到 40 个时提示，最多保留 50 个；预计新增后超过上限时拒绝新增并保留当前页面，不自动关闭或回收已有页面。40/50 是可校准的产品阈值，不构成页面性能保证。
- 页签关闭必须释放页面资源并执行关闭守卫。单页渲染或动态模块加载失败由页签级错误边界隔离；关闭故障页必须额外确认未保存内容可能无法恢复。应用内整页刷新先执行所有存活页签的正常关闭守卫，全部通过后再以整页刷新语义统一确认故障页风险；任一步拒绝或异常都不得刷新。本能力不承诺恢复故障页本地状态，也不自动重试或重放业务命令。

应用导航区分启动和用户主动操作：启动指定应用失败时回退应用列表并反馈一次；用户主动打开失败时保留当前有效页面并反馈一次；已加载应用直接激活，不重复请求；被更新请求取代的结果不得改变当前页面或补发过时提示。导航服务只返回结构化结果并负责状态迁移，具体入口负责一次普通反馈；认证和 CSRF 等专属安全处理仍由请求层负责。

个人消息中心是显式白名单内置 Header 页签 `builtin:inbox`，与业务应用共用切换、关闭和固定展示，但不创建业务应用工作台或借用应用访问权限；服务端保存当前用户的固定偏好。具体预览、筛选和详情交互见[站内消息](../platform/message/inbox.md)。
...
图标配置使用稳定的 Ant Design 组件名称。构建从 `@ant-design/icons` 实际依赖的锁定 `@ant-design/icons-svg` 版本生成内容哈希化的纯 JSON 矢量资源，完整保留 Outlined、Filled、TwoTone 名称集合、图形结构和主辅色槽；不得合并枚举依赖树中的多个图标版本。应用正常展示前完成资源接收、解析、结构验证和名称索引，图标组件首次挂载不再发起资源请求。全集只作为普通数据常驻，不得预创建全量 SVG DOM、ReactElement、组件工厂、加载闭包、订阅集合或逐图标请求状态；统一渲染器只为当前已挂载实例生成 SVG 节点。

单色图标默认继承 `currentColor`；TwoTone 保持锁定版本的默认主辅色、具体主色的官方等价辅色推导以及显式双色实例隔离。图形数据只允许受控元素和属性，不插入任意 SVG 字符串，不为图标放宽 CSP。资源初始化失败必须作为可重试的独立系统资源错误呈现，不能用固定超时放行后再显示空白、逐图标加载指示或错误图标。`IconSelector` 每页展示 24 个候选，搜索和风格筛选只操作已经就绪的名称索引；未知名称使用调用方默认图标。静态界面按钮仍按具体官方 React 组件导入，由构建去重和 tree shaking 处理。
...
自定义页面仍须优先复用通用页面壳层。除控制台、大屏等特殊交互外，主要操作位于顶部操作区，表单和业务数据以白色折叠面板组织；警告、提示与状态说明按[页面与分区提示](../development/frontend-page-guide.md#页面与分区提示)就近展示，不要求另建折叠卡片。控件密度、间距和只读表现参照通用编辑页，避免形成孤立的页面视觉体系。

`OperationType` 使用 `ADDNEW`、`EDIT` 和 `VIEW`。列表页优先将 `number` 作为第一列可点击标识；无业务编码的日志或记录可以使用更合适的标识。

## 单据页签生命周期

`EditPage` 为每个挂载实例生成稳定 Form 名称，隔离同名字段的 DOM ID、标签关联与错误定位；临时页签晋升不改变实例名称。保存或提交从校验开始到回调完成统一冻结编辑，外部 `saving` 同样生效：标准字段禁用、正文 inert，分区 `content/extra` 收到 `editable=false`，扩展命令禁用。领域自定义编辑入口必须消费该 editable 参数；Portal 选择器在冻结时撤销已打开入口。失败后恢复编辑并保留输入与脏状态，成功仍同步服务端新快照，不能通过拦截 dirty 回填把旧内容与新版本拼接。
