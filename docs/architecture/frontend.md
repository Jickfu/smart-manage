...
React 和数据请求库保持独立公共分组。其余第三方依赖仅对 `$initial`（本来已被入口静态引用的模块）按组件运行库与其余基础依赖归并，以减少初始请求数；图表、代码编辑器及未使用的组件继续按需加载。分块优先比较典型访问路径的请求数、去重后的体积和缓存复用，不能仅以 `dist` 文件总数或单个入口文件大小判断性能。动态图标保留按名称加载，避免为了减少总文件数提前下载完整图标库。
