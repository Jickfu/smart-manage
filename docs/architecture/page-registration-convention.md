...
桌面管理端 `smart-manage-web` 中，具有工作台页面入口的业务模块或单据目录使用一个 `pageRegistration.ts` 作为模块级页面白名单清单。页面组件仍保持一文件一个页面组件，注册清单可以声明列表、编辑和自定义等多个页面入口。
...
export default definePageRegistrations([
  {
    ...
    componentKey: 'demo/procurement/purchase-requisition',
    title: '采购申请',
    pageType: 'LIST',
  ...
  {
    featureKey: 'demo/procurement/purchase-requisition',
    componentKey: 'demo/procurement/purchase-requisition/edit',
    title: '采购申请',
    pageType: 'EDIT',
