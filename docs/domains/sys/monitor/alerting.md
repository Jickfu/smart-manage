...
规则支持启用状态、严重程度、触发阈值、持续时间、恢复阈值、重复通知间隔、邮件开关和接收用户。接收人引用系统用户，保存只提交用户 ID；启用邮件时，接收用户必须启用并已配置邮箱。默认规则不开启邮件，避免未完成 SMTP 和接收人配置时产生错误外发。

## 状态与并发

状态流转为 `PENDING → FIRING → RECOVERED`：首次超过阈值进入 PENDING，持续满足配置时间后进入 FIRING，低于恢复阈值后恢复。首次 FIRING 立即产生通知；持续异常只按重复间隔产生通知；恢复产生独立恢复通知。
