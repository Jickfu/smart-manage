...
- 领域是分类的展示和归属维度；分类只能直接归属于一个领域，分类本身不嵌套。
...
- `sys/base/basic-data` 使用左树右表：左树显示领域和基础资料分类，右表显示具体资料。
- 左树上方提供分类新增、编辑和删除；新增、编辑分类使用通用弹框编辑页。
- 只有选择分类后才能新增基础资料；新增页将所选分类通过禁用态引用选择器固定展示。
- 基础资料新增、编辑和查看使用工作台通用编辑页；所属分类使用禁用态引用选择器，上级资料使用树表引用选择器，只展示当前分类并排除当前资料及其后代。
