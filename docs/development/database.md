...
DEMO 的 `V3__demo_data.sql` 在显式装配 `with-demo` 时自动添加领导层、财务部、销售部三个演示部门，均归属平台默认公司。默认平台不创建这些部门；DEMO 只添加自有演示记录，不覆盖平台行，不修改管理员密码。当前仍不包含采购演示单据。演示数据随 Demo 的 Flyway 历史只执行一次，删除源码不撤销已写入的数据。
