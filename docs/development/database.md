...
领域迁移按标识稳定排序，不支持迁移依赖图。领域代码可以显式依赖其他领域的稳定 Contract（例如 demo 请假依赖 workflow），但当前 SQL 链必须能够在平台之后独立执行，不能假设其他领域已先迁移，也不建立跨领域表外键。平台不得声明业务专属目录、业务键或迁移。每条链使用独立历史表和版本空间，不能合并到同一 `spring.flyway.locations`。
