...
```text
biz/{cloudNumber}/{appNumber}/{resourceNumber}/{yyyy}/{MM}/{shard}/{uuid}.{ext}
asset/{cloudNumber}/{appNumber}/{resourceNumber}/{yyyy}/{MM}/{shard}/{uuid}.{ext}
```
...
`bizType` 一旦投入使用不得随 Java 包名、页面路径或显示名称任意改名；确需替换时新增类型并通过迁移维护历史映射。
...
- `ACTIVE`：已经绑定 `biz_type + biz_id`，读取和管理权限继承所属业务对象。
...
V29 已将历史路径映射到 `object_key`，V31 已删除含义重复的 `stored_name`、`stored_path` 和 `is_temp`；状态只以 `status` 为准。
