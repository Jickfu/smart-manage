...
本节密钥由管理端创建凭据自动生成，与密码传输 SM2 密钥对不同；其数据库密文依赖部署级 SM4 密钥。生产初始化和平台密钥变更见[生产密钥生成与部署](../../development/production-keys.md)。
...
调用监控的近 24 小时调用、成功、平均耗时和活跃应用统计显示在列表工具栏；可折叠过滤区只显示结果条件。
