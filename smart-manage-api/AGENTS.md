...
- Contract 只在出现真实跨领域消费者时，按当前用例提取最小稳定能力；没有真实跨领域调用方时禁止为预测未来需求提前创建 `contract` 包或 API。
...
- Spring Boot Parent / BOM 已管理的依赖版本不得在项目中重复显式声明。
- Spring Boot 未管理、由项目主动选择的第三方依赖版本必须统一声明在 `<properties>` 中，`dependency` 或插件配置中通过
  `${xxx.version}` 引用，禁止直接硬编码版本号。
- 同一组件族的多个 Artifact 必须共用同一个版本属性，禁止分别维护版本号。
- 只有存在明确兼容性、安全或功能原因时才允许覆盖 Spring Boot 管理的版本；覆盖必须是显式、有意的，并说明原因。
- Maven Plugin 或 annotation processor 需要显式版本时，优先复用 Spring Boot Parent 已提供或项目统一定义的版本属性，禁止创建第二个版本来源。
...
- JetCache `REMOTE` 缓存自定义对象时，所属模块必须使用项目实际 value encoder 对非空、字段完整的真实返回值执行至少一次编码测试，禁止只通过检查 `Serializable` 声明代替实际编码验证。
