...
class DualFlywayMigrationPostgresTests {
    ...
    void springBootInitializerRunsIndependentV1ChainsBeforeStartupCompletes() {
        context().run(context -> {
            ...
            assertEquals(1, versionCount("flyway_business_schema_history", "1"));
            assertEquals(1, versionCount("flyway_business_schema_history", "0"));
    ...
    void existingPlatformDataSurvivesBusinessBootstrapAndBothChainsAdvanceIndependently() throws Exception {
        ...
        assertEquals(3, rowCount("flyway_business_schema_history"));
    ...
    void checksumDriftAndOutOfOrderScriptsFailInsteadOfRepairingOrSkipping() throws Exception {
        ...
        assertThrows(FlywayException.class, () -> strategy().migrate(platform()));
        assertEquals(0, versionCount("flyway_business_schema_history", "2"));
    }
    ...
    void zeroVersionCannotBeSilentlySkippedByBootstrap() throws Exception {
        ...
        assertNull(jdbc.queryForObject("SELECT to_regclass(?)::text", String.class,
                schema + ".flyway_business_schema_history"));
    }

    private ApplicationContextRunner context() {
        return new ApplicationContextRunner()
                .withConfiguration(AutoConfigurations.of(FlywayAutoConfiguration.class))
                .withUserConfiguration(FlywayMigrationConfig.class)
                ...
                .withPropertyValues("spring.flyway.locations=" + location(platformDirectory),
                        ...
                        "smart-manage.infrastructure.migration.business.enabled=true",
                        "smart-manage.infrastructure.migration.business.location=" + location(businessDirectory),
                        "smart-manage.infrastructure.migration.business.minimum-platform-version=1");
    }
    ...
    private DualFlywayMigrationStrategy strategy() {
        BusinessMigrationProperties properties = new BusinessMigrationProperties();
        properties.setEnabled(true);
        properties.setMinimumPlatformVersion("1");
        properties.setLocation(location(businessDirectory));
