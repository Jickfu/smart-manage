...
public class MonitorRoutingGateway {
  ...
  public <T> T get(
      MonitorInstanceRegistry.RegisteredInstance instance, String path, Class<T> responseType) {
  ...
  private HttpRequestSpec.Builder requestBuilder(
      MonitorInstanceRegistry.RegisteredInstance instance, String path, String method) {
    HttpRequestSpec.Builder builder =
        HttpRequestSpec.builder(method, URI.create(instance.getInternalBaseUrl() + path))
            .timeout(Duration.ofMillis(properties.getCluster().getRequestTimeoutMs()))
    ...
    try {
      ...
      if (root.path("code").asInt(-1) != 0) {
        throw new BizException(
            ResultEnum.EXTERNAL_SERVICE_ERROR, root.path("msg").asText("目标实例诊断失败"));
      }
