    ...
    if ("HOST".equals(scopeType))
      return jdbcTemplate.query(
          "SELECT date_bin(INTERVAL '"
              ...
              + " cpu_usage,avg(memory_used/nullif(memory_total,0))"
              + " memory_usage,max(worst_filesystem_usage) filesystem_usage,max(worst_mount)"
    ...
    if ("INSTANCE".equals(scopeType))
      return jdbcTemplate.query(
          "SELECT date_bin(INTERVAL '"
              ...
              + " process_cpu,avg(heap_used/nullif(heap_max,0)) heap_usage,avg(http_request_rate)"
              + " http_request_rate,avg(http_5xx_rate) http_5xx_rate,avg(http_p95_ms)"
              + " http_p95_ms,avg(http_p99_ms) http_p99_ms,avg(thread_count)"
              + " thread_count,avg(blocked_thread_count)"
              + " blocked_thread_count,avg(db_active/nullif(db_max,0))"
