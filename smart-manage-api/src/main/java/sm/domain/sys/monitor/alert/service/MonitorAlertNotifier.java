...
""",
      ...
      List<Long> recipients =
          jdbcTemplate.queryForList(
              ...
              """,
              Long.class,
              ((Number) notification.get("incident_id")).longValue());
      ...
      String subject =
          "[Smart Manage]["
              ...
              + "] "
              + ("RECOVERY".equals(type) ? "已恢复：" : "告警：")
              + notification.get("rule_name");
      String text =
          notification.get("summary")
              + "\n对象："
              + notification.get("scope_type")
              + "/"
      ...
      Long taskId =
          emailSender.enqueue(
              new EmailNotificationCommand(
                  "monitor.alert",
                  "monitor.alert:" + notificationId,
                  recipients,
                  subject,
                  html,
                  text));
      jdbcTemplate.update(
          "UPDATE t_sys_monitor_alert_notification SET"
              + " status='SUCCESS',email_task_id=?,completed_time=now(),error_message=NULL WHERE"
              + " id=? AND status='PROCESSING'",
          taskId,
          notificationId);
...
""");
  ...
  private void markSkipped(Long id, String reason) {
    jdbcTemplate.update(
        "UPDATE t_sys_monitor_alert_notification SET"
            + " status='SKIPPED',completed_time=now(),error_message=? WHERE id=?",
        reason,
        id);
  }
