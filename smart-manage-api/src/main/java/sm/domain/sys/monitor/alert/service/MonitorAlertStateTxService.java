...
class MonitorAlertStateTxService {
    ...
    void evaluate(MonitorAlertEvaluation evaluation) {
        ...
        List<Map<String,Object>> active = jdbcTemplate.queryForList("""
                ...
                """, evaluation.ruleId(), evaluation.scopeType(), evaluation.scopeId());
        if (active.isEmpty()) {
        ...
        boolean stillAbnormal = evaluation.recoveryThreshold() == null
        ...
        if (!stillAbnormal) {
            jdbcTemplate.update("""
                    ...
                    """, jdbcTime(now), jdbcTime(now), lastValue, peak, evaluation.summary(), incidentId);
            if ("FIRING".equals(status) && evaluation.emailEnabled()) enqueue(incidentId, "RECOVERY", 1, now);
            return;
        }
        Instant startedAt = instant(incident.get("started_at"));
        if ("PENDING".equals(status) && !now.isBefore(startedAt.plusSeconds(evaluation.durationSeconds()))) {
            jdbcTemplate.update("""
                    ...
                    """, jdbcTime(now), jdbcTime(now), lastValue, peak, evaluation.summary(), evaluation.emailEnabled() ? jdbcTime(now) : null,
                    evaluation.emailEnabled() ? 1 : 0, incidentId);
        ...
        boolean repeat = "FIRING".equals(status) && evaluation.emailEnabled()
                && (lastNotifiedAt == null || !now.isBefore(lastNotifiedAt.plusSeconds(evaluation.repeatIntervalSeconds())));
        jdbcTemplate.update("""
                ...
                """, jdbcTime(now), lastValue, peak, evaluation.summary(), repeat ? jdbcTime(now) : jdbcTime(lastNotifiedAt),
                repeat ? notificationCount + 1 : notificationCount, incidentId);
        if (repeat) enqueue(incidentId, lastNotifiedAt == null ? "FIRING" : "REPEAT", notificationCount + 1, now);
    }
    ...
    private void createPending(MonitorAlertEvaluation evaluation, Instant now) {
        ...
        jdbcTemplate.update("""
                ...
                VALUES(?,?,?,?,?,'PENDING',?,?,?,?,?,?,0,?,0)
                """, incidentId, evaluation.ruleId(), evaluation.ruleCode(), evaluation.scopeType(), evaluation.scopeId(),
                cycleKey, jdbcTime(now), jdbcTime(now), evaluation.value(), evaluation.value(), evaluation.threshold(), evaluation.summary());
        if (evaluation.durationSeconds() == 0) {
            jdbcTemplate.update("""
                    ...
                    """, jdbcTime(now), evaluation.emailEnabled() ? jdbcTime(now) : null, evaluation.emailEnabled() ? 1 : 0, incidentId);
            if (evaluation.emailEnabled()) enqueue(incidentId, "FIRING", 1, now);
        }
    }
    ...
    private void enqueue(long incidentId, String type, int sequence, Instant now) {
        jdbcTemplate.update("""
                ...
                """, IdWorker.getId(), incidentId, type, sequence, jdbcTime(now), jdbcTime(now));
    }

    private BigDecimal decimal(Object value) { return value instanceof BigDecimal decimal ? decimal : value == null ? null : new BigDecimal(value.toString()); }
    private BigDecimal max(BigDecimal first, BigDecimal second) { if (first == null) return second; if (second == null) return first; return first.max(second); }
